...
(function(){
  ...
  @media(max-width:600px){.dash-grid{grid-template-columns:1fr}.dash-pie-wrap{grid-template-columns:1fr}.dash-pie{margin:auto}.dash-title{align-items:flex-start;flex-direction:column}.dash-chart{height:265px}}
  `;
  ...
  function trend(rows){if(rows.length<2)return null;const first=rows[0].total,last=rows[rows.length-1].total;return {value:last-first,pct:first?((last-first)/first*100):0,first,last}}
  function lineChart(rows){
    ...
    const w=760,h=285,left=58,right=18,top=22,bottom=48,plotBottom=h-bottom,plotH=plotBottom-top;
    const vals=rows.map(r=>r.total),min=Math.min(...vals),max=Math.max(...vals),span=max-min||1;
    const pts=rows.map((r,i)=>{const x=left+i*(w-left-right)/(rows.length-1);const y=plotBottom-(r.total-min)/span*plotH;return [x,y,r]});
    const poly=pts.map(x=>x[0]+','+x[1]).join(' ');
    const area=`${left} ${plotBottom} ${pts.map(x=>x[0]+' '+x[1]).join(' ')} ${w-right} ${plotBottom}`;
    const mid=(max+min)/2;
    const grid=[max,mid,min];
    const gridSvg=grid.map((v,i)=>{const y=plotBottom-(v-min)/span*plotH;return `<line x1="${left}" y1="${y}" x2="${w-right}" y2="${y}" class="dash-gridline"></line><text x="${left-9}" y="${y+4}" text-anchor="end" class="dash-axis">${dashMoney(v)}</text>`}).join('');
    const xLabels=pts.map((x,i)=>i===0||i===pts.length-1||pts.length<=5?`<text x="${x[0]}" y="${h-12}" text-anchor="${i===0?'start':i===pts.length-1?'end':'middle'}" class="dash-chart-label">${dashDate(x[2].d)}</text>`:'').join('');
    const pointLabels=pts.map((x,i)=>i===0||i===pts.length-1?`<text x="${x[0]}" y="${x[1]-10}" text-anchor="${i===0?'start':'end'}" class="dash-chart-value">${dashMoney(x[2].total)}</text>`:'').join('');
    return `<svg viewBox="0 0 ${w} ${h}" preserveAspectRatio="none" role="img" aria-label="Evolución del patrimonio">${gridSvg}<polygon points="${area}" fill="#edf3ff" opacity=".75"></polygon><polyline points="${poly}" fill="none" stroke="#234b86" stroke-width="3.5" stroke-linecap="round" stroke-linejoin="round"></polyline>${pts.map(x=>`<circle class="dash-point" cx="${x[0]}" cy="${x[1]}" r="5" fill="#fff" stroke="#234b86" stroke-width="2.5"><title>${dashDate(x[2].d)} · ${dashMoney(x[2].total)}</title></circle>`).join('')}${pointLabels}${xLabels}</svg>`;
  }
  ...
  function summaryDashboard(){
    ...
    const trendHtml=tr?`<div class="dash-trend"><span class="dash-trend-badge ${tr.value>=0?'up':'down'}">${tr.value>=0?'▲':'▼'}</span><span class="${tr.value>=0?'dash-positive':'dash-negative'}">${tr.value>=0?'+':'−'}${dashMoney(Math.abs(tr.value))} · ${Math.abs(tr.pct).toFixed(2)} %</span><span class="dash-small" style="margin:0">en el periodo mostrado</span></div>`:'';
    return `<div class="dash">
      ...
      <div class="dash-layout">
        <div class="dash-card"><div class="dash-title"><h3>Evolución del patrimonio</h3><div class="dash-filters">${ranges.map(([k,l])=>`<button class="dash-filter ${PatrimonioDashboard.range===k?'active':''}" onclick="PatrimonioDashboard.range='${k}';render()">${l}</button>`).join('')}</div></div>${lineChart(rows)}${trendHtml}</div>
