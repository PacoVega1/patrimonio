...
function findOperationsTable(){
 ...
 return tables.find(t=>{
   const hs=[...t.querySelectorAll('th')].map(x=>norm(x.textContent));
   const text=norm(t.innerText||'');
   return (hs.includes('fecha')||text.includes('fecha')) &&
          (text.includes('aportaciones anteriores')||text.includes('nº aportaciones')) &&
...
function getOps(){
 ...
 if(window.S.context)ops=ops.filter(x=>x.producto_id===window.S.context);
...
function rebuild(){
 ...
 rows.forEach(r=>{
   ...
   vals.forEach((v,i)=>{const td=document.createElement('td');td.textContent=v;td.style.textAlign=i===0||i===7?'left':'right';tr.appendChild(td)});
...
let timer;
function apply(){try{rebuild();fixDates();}catch(e){console.error('ui-fixes Operaciones:',e)}}
const observer=new MutationObserver(()=>{clearTimeout(timer);timer=setTimeout(apply,200)});
function start(){observer.observe(document.body,{childList:true,subtree:true});apply();setTimeout(apply,500);setTimeout(apply,1200);setTimeout(apply,2500);}
